...
namespace JDI.Light.Elements.Complex
{
        ...
        where TEnum : IConvertible
    {
        protected Action<Menu<TEnum>, string[], Action<IWebDriver, IWebElement>> ChooseItemAction =
            (m, names, action) =>
            {
                ...
                for (var i = 0; i < nodes.Count; i++)
                {
                    ...
                    if (element == null)
                        throw JDI.Assert.Exception("Can't choose element:" + value);
        ...
        public void Hover(params string[] names)
        {
            if (names == null || names.Length == 0)
                return;
            Invoker.DoActionWithWait($"Hover '{names.FormattedJoin(Separator)}'", () => ChooseItemAction(this, names, HoverAction));
        }
        ...
        {
            Invoker.DoActionWithWait($"Select '{names.FormattedJoin(Separator)}'", () => HoverAndClickAction(this, names));
        }
