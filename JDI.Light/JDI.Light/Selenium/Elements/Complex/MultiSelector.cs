        ...
        where TEnum : IConvertible
    {
        ...
        public Action<MultiSelector<TEnum>> ClearAction = m =>
        {
            if (!m.HasLocator && m.AllLabels == null)
                throw JDISettings.Assert.Exception("Can't clear options. No optionsNamesLocator and allLabelsLocator found");
            ...
            : base(optionsNamesLocator, allLabelsLocator)
        {
            ...
            GetWebElementFunc = (s, name) =>
            {
                ...
                if (!ms.HasLocator && ms.AllLabels == null)
                    throw JDISettings.Assert.Exception("Can't get option. No optionsNamesLocator and allLabelsLocator found");
        ...
        {
            if (num <= 0)
                throw JDISettings.Assert.Exception($"Can't get option with num '{num}'. Number should be 1 or more");
            if (num > els.Count)
                throw JDISettings.Assert.Exception($"Can't get option with num '{num}'. Found only {els.Count} options");
            return els[num - 1];
        }
