...
namespace JDI.Light.Selenium.Elements.Complex
{
        ...
        where TEnum : IConvertible
    {
        ...
        {
            if (!s.HasLocator)
                throw JDISettings.Assert.Exception("Element has no locators");
        ...
        private void SelectFromList(IList<IWebElement> els, string name)
        {
            ...
            if (element == null)
                throw JDISettings.Assert.Exception($"Can't find option '{name}'. Please fix _allLabelsLocator");
            element.Click();
        }
        ...
        {
            if (num <= 0)
                throw JDISettings.Assert.Exception($"Can't get option with num '{num}'. num should be 1 or more");
            if (els == null)
                throw JDISettings.Assert.Exception($"Can't find option with num '{num}'. Please fix _allLabelsLocator");
            if (els.Count < num)
                throw JDISettings.Assert.Exception($"Can't find option with num '{num}'. Find only '{els.Count}' options");
            els[num - 1].Click();
        }
        ...
        private bool DisplayedInList(IList<IWebElement> els, int num)
        {
            ...
            if (els == null)
                throw JDISettings.Assert.Exception($"Can't find option with num '{num}'. Please fix _allLabelsLocator");
            if (els.Count < num)
                throw JDISettings.Assert.Exception($"Can't find option with num '{num}'. Find '{els.Count}' options");
